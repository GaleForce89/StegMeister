...
public class Encrypt extends Cryptography {
  ...
  public String Crypt(
      String msg, String test) { // DOES NOT WORK, NEED TO CORRECTLY CONVERT BYTE ARRAY TO STRING
    /*
    ...
    // try using cipher block chaining, need to create a random initialization vector so ciphertext
    // is different
    // with each encryption
    byte[] initVector = new byte[16]; // create byte array to fill initialization vector
    SecureRandom randomNumber =
        new SecureRandom(); // new random number to randomize initialization vector
    randomNumber.nextBytes(initVector); // fill byte array with random numbers
    IvParameterSpec ivpspec =
        new IvParameterSpec(initVector); // set up initialization vector with random byte array
    ...
    try {
      ...
      strBuilder.append(
          Arrays.toString(
              out)); // *****this doesn't work, look into using base64 encoder for conversion*****
